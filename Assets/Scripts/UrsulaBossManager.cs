...
    [Header("生物排列設定")]
    [SerializeField] private float formationForwardDist = 2.0f;
...
       // StopSpellEffect(); // 確保攻擊粒子在非攻擊階段不播放
        StartCoroutine(BossLoop());
        SetupCreatureFormation();
    }
    ...
    private void SetupCreatureFormation()
    {
        BioInstrument[] bios = FindObjectsOfType<BioInstrument>();
        System.Array.Sort(bios, (a, b) => a.instrumentID.CompareTo(b.instrumentID));
        for (int i = 0; i < bios.Length; i++)
        {
            CreatureFollower follower = bios[i].GetComponent<CreatureFollower>();
            if (follower != null)
                follower.SetFormationFollow(i, bios.Length, formationForwardDist, formationSpacing);
        }
    }
...
            StartSpellEffect();
            while (isDefenseWindow && timer < 10f) {
...
        //if (poofParticle != null) poofParticle.Stop();
    }
...
        bossHP--;
        Debug.Log("poofParticle is " + (poofParticle == null ? "NULL - 請在 Inspector 指定!" : poofParticle.name));
        if (poofParticle != null) { 
            poofParticle.transform.position = realUrsula.transform.position; poofParticle.Stop(); poofParticle.Clear(); poofParticle.Play(); }
...
        }
        if (kissButton != null) {
            kissButton.SetActive(true);
            Debug.Log("顯示親吻測試按鈕 (Btn_Kiss)");
        }
